//pUI_robot.pde
// code for parking robot


import com.martinleopold.pui.*;
import com.martinleopold.pui.events.*;
/*
...
//  ui.padding(0.25, 0.25); // set padding (in grid units)
  ...
  ui.newRow();
  ui.addToggle().label("forward").calls("toggleToggled");
  ui.addToggle().label("back").sets("t2Value");
  ui.newRow();
  ui.addToggle().label("ccw");
  ui.addToggle().label("cc");
  ui.addDivider();  
...
//  ui.addSlider().label("s2").sets("s2Value");
  ...
  ui.newRow();
  ui.addButton().label("sweep").calls("buttonClick");
