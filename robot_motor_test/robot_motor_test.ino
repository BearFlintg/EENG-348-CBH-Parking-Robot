/* Robot testing code
EENG 348 Oct 7 2014
Bear Flintgruber, Hope Wilson, Chris Datkakis
Connections:

(Ard)   (Robot)
Pin 2-> pin 4, Left wheel encoder
Pin 3-> pin 5, Right wheel encoder
Pin 6-> pin 6, Fin, Right motor
Pin 7-> pin 7, Rin, Right motor
Pin 8-> pin 8, Fin, Left motor
Pin 9-> pin 9, Rin, Left motor

ADC4->Potentiometer 0, Right motor (DAC B)
ADC5->Potentiometer 1, Left motor (DAC A)

*/
int ADC4_B, ADC5_A, direct,i,val;

// added by Hope
#define CLOCK 10
#define DOUT 11
#define CSLD 12
#define NBITS 16
int b[NBITS];


void setup(){
  Serial.begin(115200); //open serial port, set BR
  //ZxSerial.setTimout(10); //make parseInt() faster
  pinMode(6, OUTPUT); //set motor control pins to output
  pinMode(7,OUTPUT);
  pinMode(8,OUTPUT);
  pinMode(9,OUTPUT);
  digitalWrite(6,LOW); //set all low--off
  digitalWrite(7,LOW);
  digitalWrite(8,LOW);
  digitalWrite(9,LOW);
  //added by Hope
  pinMode(CLOCK, OUTPUT);
  digitalWrite(CLOCK, LOW);
  pinMode(DOUT, OUTPUT);
  digitalWrite(DOUT, LOW);
  pinMode(CSLD, OUTPUT);
  digitalWrite(CSLD, HIGH);
  
  
   //set timer1 interrupt at 1Hz
  TCCR1A = 0;// set entire TCCR1A register to 0 (clear)
  TCCR1B = 0;// same for TCCR1B 
  TCNT1  = 0;//initialize counter value to 0
  // set compare match register for 1hz increments
  OCR1A = 31250;// = (16*10^6 Hz) / (2 sec*1024 prescale) - 1 
                //(must be <65536 on Timer 1)
  TCCR1B |= (1 << WGM12); // turn on CTC mode
  TCCR1B |= (1 << CS12);  // Set CS12 bit to 1 for 256 prescaler
  TCCR1B |= (1 << CS10);  //Set SC10 bit to 1 for 1024 prescaler
  TIMSK1 |= (1 << OCIE1A); // enable timer compare interrupt
 
 direct=1; 
}

void I2DACb(int x, int potval) {
  if (potval == 4) // potB
  { // code for Load DAC B is 0010
    b[0] = 1;
    b[1] = 0;
    b[2] = 1;
    b[3] = 0;
  }
  else // potA
  { // code for Load DAC A is 0001
    b[0] = 1;
    b[1] = 0;
    b[2] = 0;
    b[3] = 1;    
  }
  
 for (int i = 13; i >= 4; i--) { // MSB
   b[i] = x % 2;
   x = x / 2;
 } 
 
 b[14] = 0;
 b[15] = 0;
 for (i = 0; i < 16; i++)
 {
  Serial.print(b[i]); 
 }
 Serial.println();
}

void Send2DAC(int ADC4_B, int ADC5_A){
  I2DACb(ADC4_B, 4);
  digitalWrite(CSLD, LOW);
  for (i = 0; i < NBITS; i++)
  {
    digitalWrite(CLOCK, 0);
    delay(1);
    digitalWrite(DOUT, b[i]);
    delay(1);
    digitalWrite(CLOCK, 1);  
    delay(1); 
  }
  delay(1);
  digitalWrite(CSLD, HIGH);  
  delay(1);
  I2DACb(ADC5_A, 5);
  
  digitalWrite(CSLD, LOW);  
  for (i = 0; i < NBITS; i++)
  {
    delay(1);
    digitalWrite(CLOCK, 0);
    delay(1);
    digitalWrite(DOUT, b[i]);
    delay(1);
    digitalWrite(CLOCK, 1);
  }
  delay(10);
  digitalWrite(CSLD, HIGH);  
}

void ChangeDirection( int direct){
  Serial.println("w/in function");
  Serial.print("direct=");
  Serial.println(direct);
  
  if(direct==1){
    digitalWrite(6,LOW); // Forward
    digitalWrite(7,HIGH); 
    digitalWrite(8,HIGH);
    digitalWrite(9,LOW);
  }
  else if (direct==0){
  digitalWrite(6,HIGH); //Reverse
  digitalWrite(7,LOW);
  digitalWrite(8,LOW);
  digitalWrite(9,HIGH);
  }
 /* direct=(direct+1)%2;
  Serial.print("Direct at end: ");
  Serial.println(direct); */
}

ISR(TIMER1_COMPA_vect){
  Serial.println("Timer Interrupt");
  ADC4_B=analogRead(4);
  ADC5_A=analogRead(5);
  //Serial.println("End interrupt");
  //ChangeDirection(direct);
  
}

void loop(){
  /*Serial.println("Starting routine");
  
  Serial.println("Forward!!!");
  
  digitalWrite(6,LOW); //set Rin for both high Current from 2 to 1
  digitalWrite(7,HIGH); 
  digitalWrite(8,HIGH);
  digitalWrite(9,LOW);
  delay(2000);
  
  Serial.println("Reverse! Reverse!");
  digitalWrite(6,HIGH); //set all High--off
  digitalWrite(7,LOW);
  digitalWrite(8,LOW);
  digitalWrite(9,HIGH);
  delay(2000);
  
  Serial.println("Changing direction");
  ChangeDirection(direct);
  direct=(direct+1)%2;
  delay(2000);*/
  
  Serial.println("Changing direction");
  ChangeDirection(direct);
  direct=(direct+1)%2;
  delay(2000);
  
  /*Serial.println("Off");
  digitalWrite(6,LOW); //set all High--off
  digitalWrite(7,LOW);
  digitalWrite(8,LOW);
  digitalWrite(9,LOW);
  digitalWrite(13,LOW);
  delay(2000);
 
  
  Serial.print("ADC4_B: ");
  Serial.println(ADC4_B);
  Serial.print("ADC5_A: ");
  Serial.println(ADC5_A);*/
  
  Send2DAC(ADC4_B, ADC5_A); 
  
  
}
